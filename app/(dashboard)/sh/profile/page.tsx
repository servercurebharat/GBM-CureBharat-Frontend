  ...
  return (
    <DashboardLayout pageTitle="Profile">
      <div className="space-y-6 pb-12">
            ...
            <div className="flex-1 space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6 pt-2">
                <div>
                  ...
                  <p className="text-sm font-bold text-white flex items-center gap-1.5 hover:text-[#60A5FA] cursor-pointer transition-colors">
                    {typeof user.referrerId === 'object' ? user.referrerId?.memberId : 'N/A'}
                  </p>
                </div>
                <div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {[
                ...
                { label: 'Street Address', value: user.address?.street || 'N/A' },
                { label: 'City', value: user.address?.city || 'N/A' },
                { label: 'Zip Code', value: user.address?.zipCode || 'N/A' },
              ].map((field, i) => (
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {[
                ...
                { label: 'Branch Name', value: user.bankDetails?.branchName || 'N/A' },
              ].map((field, i) => (
                <div key={i} className="space-y-2">
                  <label className="text-[10px] font-bold text-[#B5B8BD] uppercase tracking-widest pl-1">{field.label}</label>
                  <div className="bg-white/2 border border-white/5 rounded-xl px-4 py-3 text-sm font-bold text-white shadow-inner">{field.value}</div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {[
                ...
                { label: 'Nominee Mobile', value: user.nomineeDetails?.mobile || 'N/A' },
              ].map((field, i) => (
                <div key={i} className="space-y-2">
                  <label className="text-[10px] font-bold text-[#B5B8BD] uppercase tracking-widest pl-1">{field.label}</label>
                  <div className="bg-white/2 border border-white/5 rounded-xl px-4 py-3 text-sm font-bold text-white shadow-inner">{field.value}</div>
        ...
        {/* 4. Document Vault */}
        <div className="bg-[#131241] rounded-[24px] p-8 shadow-2xl border border-white/5 space-y-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-sm font-bold text-white uppercase tracking-wider">KYC Documents</h3>
            </div>
          </div>
             ...
             <div className="bg-white/2 rounded-xl overflow-hidden border border-white/5">
               <table className="w-full text-left text-[10px]">
                 <thead className="bg-white/5 border-b border-white/5">
                   <tr><th className="px-6 py-3 font-bold text-[#B5B8BD] uppercase">Document Type</th><th className="px-6 py-3 font-bold text-[#B5B8BD] uppercase">Details</th></tr>
                 </thead>
                 <tbody className="divide-y divide-white/5 font-medium text-slate-300">
                    <tr><td className="px-6 py-4">Aadhaar Card</td><td className="px-6 py-4">{user.kycDocuments?.aadhaarNumber || 'N/A'}</td></tr>
                    <tr><td className="px-6 py-4">PAN Card</td><td className="px-6 py-4">{user.kycDocuments?.panNumber || 'N/A'}</td></tr>
                    <tr><td className="px-6 py-4">Bank IFSC</td><td className="px-6 py-4">{user.bankDetails?.ifscCode || 'N/A'}</td></tr>
                 </tbody>
               </table>
             </div>
          </div>
        ...
        {user.kycStatus !== 'approved' && (
          <div className="bg-amber-400/5 border border-amber-400/20 rounded-[12px] p-6 flex items-start gap-4">
            ...
            <div>
              <p className="text-xs font-bold text-amber-400 uppercase tracking-widest mb-1">Action Required: KYC Verification Pending</p>
              <p className="text-[11px] text-slate-500 font-medium leading-relaxed">Your KYC status is currently {user.kycStatus.replace('_', ' ')}. Some features may be restricted until your documents are verified by the admin.</p>
            </div>
          </div>
        )}
      </div>
