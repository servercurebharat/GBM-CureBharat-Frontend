...
export default function HcmOverrideLedger() {
  ...
  return (
    <DashboardLayout pageTitle="Override Income Ledger">
       <div className="space-y-8 pb-20">
          <div className="flex items-center justify-between">
             <div>
                <h2 className="font-display text-2xl font-bold text-white tracking-tight">Managerial Overrides</h2>
                <p className="text-sm text-muted mt-1 font-medium">Tracking 40% commissions earned from your frontline HCC sales</p>
             </div>
             <button className="bg-white/[0.03] border border-white/10 px-4 py-2 rounded-xl text-[10px] font-bold text-white uppercase tracking-widest hover:bg-white/[0.05] transition-all">
                Export Statement
             </button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
             <div className="bg-surface border border-white/[0.07] rounded-2xl p-6">
                <p className="text-[10px] font-black text-muted uppercase tracking-widest mb-1">This Cycle Earnings</p>
                <h4 className="text-2xl font-display font-bold text-hcc">{formatAmount(stats.thisMonth)}</h4>
             </div>
             <div className="bg-surface border border-white/[0.07] rounded-2xl p-6">
                <p className="text-[10px] font-black text-muted uppercase tracking-widest mb-1">Pending Finalization</p>
                <h4 className="text-2xl font-display font-bold text-amber-500">{formatAmount(stats.pending)}</h4>
             </div>
             <div className="bg-surface border border-white/[0.07] rounded-2xl p-6">
                <p className="text-[10px] font-black text-muted uppercase tracking-widest mb-1">All-Time Overrides</p>
                ...
                <table className="w-full text-left border-collapse">
                   <thead>
                      <tr className="bg-white/[0.01] border-b border-white/[0.07]">
                         ...
                         <th className="px-6 py-4 text-[10px] font-black text-muted uppercase tracking-widest">Source HCC</th>
                         <th className="px-6 py-4 text-[10px] font-black text-muted uppercase tracking-widest">Description</th>
                   ...
                   <tbody className="divide-y divide-white/[0.04]">
                      {loading ? (
                         ...
                         <tr><td colSpan={5} className="px-6 py-20 text-center text-sm text-muted font-bold uppercase tracking-widest">No override entries found in your wallet</td></tr>
                      ) : (
                         ledger.map((entry) => (
                            <tr key={entry._id} className="hover:bg-white/[0.01] transition-colors">
                               ...
                               <td className="px-6 py-4">
                                  <div className="text-sm font-bold text-white uppercase tracking-tight">
                                     {entry.sourceUserId?.name || 'Network Sale'}
                                  </div>
                               </td>
                               <td className="px-6 py-4 text-[10px] text-muted font-medium uppercase tracking-tight max-w-xs truncate">
                                  {entry.description}
                               </td>
                               <td className="px-6 py-4 text-sm font-black text-sh tracking-tighter">
